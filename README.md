# AI Basics Experiments

This repository contains the Jupyter notebooks for Experiments 1 to 5 from the AI Basics / ENCS5141 lab work.

## Contents

- [exp1/exp1.ipynb](exp1/exp1.ipynb)
- [ENCS5141_Exp2_Data_Visualization_and_Cleaning_and_Feature_Selection.ipynb](ENCS5141_Exp2_Data_Visualization_and_Cleaning_and_Feature_Selection.ipynb)
- [ENCS5141_Exp3_Feature_Engineering.ipynb](ENCS5141_Exp3_Feature_Engineering.ipynb)
- [Exp4.ipynb](Exp4.ipynb)
- [ENCS5141_Exp5_Unsupervised_Learning.ipynb](ENCS5141_Exp5_Unsupervised_Learning.ipynb)

## Notes

- Notebook outputs and execution counts are cleared so the repo stays clean in git.
- Dataset files are kept under `ENCS5141Datasets/` for reproducibility.
- The `.vscode/` and `.venv/` folders are ignored because they are local development artifacts.